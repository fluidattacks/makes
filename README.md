...
spec:
  template:
    spec:
      containers:
        - name: example
          image: ghcr.io/fluidattacks/makes:22.01
          command: [m]
          args:
...
1. Now install Makes by running one of the following commands:
    ...
    - For Nix versions >= 2.3 and < 2.4 (nix stable)
      ...
      `$ nix-env -if https://fluidattacks.com/makes/install/22.01`

    - For Nix versions == 2.4 (nix unstable)

      `$ nix profile install github:fluidattacks/makes/22.01`
...
jobs:
  helloWorld:
    ...
    - uses: docker://ghcr.io/fluidattacks/makes:22.01
...
language: nix
nix: 2.3.12
install: nix-env -if https://fluidattacks.com/makes/install/22.01
env:
  global:
...
1. You need to generate `sourcesYaml` like this:
    ...
    ```bash
    m github:fluidattacks/makes@22.01 /utils/makePythonPypiEnvironmentSources \
      "${python_version}" \
      "${dependencies_yaml}" \
      "${sources_yaml}
    ```
