{ __nixpkgs__, __toModuleOutputs__, deployNomad, attrsMerge, attrsMapToList, ...
}:
{ config, lib, ... }:
let
  makeNamespaceOutput = namespace:
    { setup, jobs, version, }:
    attrsMapToList makeJobOutput (lib.mapAttrs (_: src: {
      inherit setup;
      inherit version;
      inherit namespace;
      inherit src;
    ...
    deployNomad = {
      ...
      namespaces = lib.mkOption {
        ...
        type = lib.types.attrsOf (lib.types.submodule (_: {
          options = {
            setup = setupOpt;
            jobs = lib.mkOption { type = lib.types.attrsOf lib.types.path; };
            version = versionOpt;
          };
        }));
      };
