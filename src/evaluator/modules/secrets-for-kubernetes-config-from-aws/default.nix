{ __toModuleOutputs__, makeSecretForKubernetesConfigFromAws, ... }:
{ config, lib, ... }:
let
  secretForKubernetesConfigFromAwsType = lib.types.submodule (_: {
    options = {
      cluster = lib.mkOption { type = lib.types.str; };
...
in {
  ...
  config = {
    outputs =
      __toModuleOutputs__ makeOutput config.secretsForKubernetesConfigFromAws;
  };
}
